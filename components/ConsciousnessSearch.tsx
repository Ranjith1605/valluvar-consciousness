    ...
    <div className="w-full max-w-2xl mx-auto relative z-20 mb-12">
      ...
      <div className={`relative rounded-full overflow-hidden border transition-all duration-300 ${
        ...
        <input 
          ...
          className={`w-full bg-transparent py-4 pl-16 pr-6 outline-none font-sans text-lg ${
            mode === 'scholar' ? 'text-white placeholder-gray-500' : 'text-cyan-50 placeholder-cyan-400/50'
          }`}
          value={query}
      ...
      {/* Results Dropdown */}
      {results.length > 0 && (
        <div className={`absolute top-full left-0 w-full mt-4 rounded-2xl overflow-hidden border backdrop-blur-xl p-2 z-50 max-h-[400px] overflow-y-auto ${
          mode === 'scholar' ? 'bg-charcoal/95 border-gray-700' : 'bg-indigo-950/95 border-cyan-500/30'
        }`}>
          {results.map(kural => (
            <button 
              key={kural.number}
              onClick={() => handleSelect(kural)}
              className={`w-full text-left p-4 rounded-xl mb-1 last:mb-0 transition-colors ${
                 mode === 'scholar' ? 'hover:bg-white/10 text-gray-300' : 'hover:bg-cyan-500/20 text-cyan-100'
              }`}
            >
